...
- ### 관리자 도메인 관리
    - 관리자 도메인 등록
        - 1, 6, 12개월 단위로 관리자 도메인을 등록할 수 있다.
        - 도메인은 고유해야 한다.
    - 관리자 도메인 갱신
    ...
    - 도메인 정보 수정
        ...
        - 이 때, 해당 도메인을 이용하는 관리자의 이메일 주소는 자동으로 변경된다.
- ### 회원 관리
    - 회원 가입
        - 필요 정보: 이메일, 비밀번호, 이름, 닉네임, 연락처
        - 이메일은 고유해야한다.
        - 비밀번호는 반드시 영어와 숫자를 포함한 8자리 이상의 문자열이여야 하며, 암호화되어 저장된다.
        - 일반적으로 일반 사용자(USER) 권한으로 가입되지만 유효한 관리자 도메인으로 등록되어있는 메일 주소를 이용하고 있다면 관리자(ADMIN) 권한으로 가입된다.
    - 로그인
...
- ### 판매 상품 관리
    - 사용자의 판매 상품 등록
        ...
        - 필요 입력 정보: 상품명, 가격, 상품 설명, 위탁 여부
            - 직접 판매
                - 상품 등록 시 상품 상태는 판매중(SELLING) 상태로 등록된다.
                - 추가 필요 입력 정보: 품질
                - 거래 방식 등록
                    - 거래 가능 정보를 입력한다.
                    - 필요 입력 정보: 거래 방식(택배(PARCEL), 반값 택배(CONVENIENCE), 직거래(MEETING)) 중 선택
                    ...
                    - 거래 방식이 등록되지 않으면 상품 조회 시 나타나지 않는다.
            - 위탁 판매
    ...
    - 관리자의 위탁 상품 관리
        ...
        - 거래 방식 등록
            - 거래 가능 정보를 입력한다.
            - 필요 입력 정보: 거래 방식(택배(PARCEL), 반값 택배(CONVENIENCE) 중 선택
            - 추가 금액은 배송비 등 거래 방식 별 발생하는 추가 금액이다(배송비, 포장비 등)
            - 거래 방식이 등록되지 않으면 상품 목록 조회 시 상품이 포함되지 않는다.
    - 판매자의 위탁 정보 등록
