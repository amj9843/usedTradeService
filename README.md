...
- ### 회원 관리
    ...
    - 계좌 관리
        - 계좌 등록
            ...
            - 이 때 은행, 계좌 번호, 소유주명이 모두 일치하는 계좌 정보가 없어야 한다.
        - 계좌 수정
            - 수정 가능 정보: 대표 계좌 설정 여부, 은행, 계좌 번호, 소유주
            - 대표 계좌 설정 여부를 변경하면 기존의 대표 계좌는 자동으로 해제된다.
            - 값을 입력하지 않거나 빈 칸 입력 시 기존의 정보가 그대로 이어진다.
        - 계좌 삭제
    ...
    - 사용자 정보 조회
        ...
        - 조회되는 정보:
            - 관리자의 식별번호일 때: 회원 식별번호, 닉네임, 권한, 판매 점수, 판매 내역, 주소
...
- ### 판매 상품 관리
    - 사용자의 판매 상품 등록
        - 로그인한 일반 사용자는 판매할 상품을 등록할 수 있다. 이 때, 판매 유형은 직접 판매(DIRECT)와 위탁 판매(CONSIGNMENT)로 나뉜다.
        - 관련된 이미지도 최대 5장까지 등록 가능하다.
            - 이미지를 등록하면 S3에 저장되고, 저장되면서 생선된 src를 DB에 등록한다.
        - 필요 입력 정보: 상품명, 가격, 상품 설명, 위탁 여부, 상품의 카테고리(상품이 해당하는 카테고리 식별번호를 LIST 형으로 입력하며, 빈 리스트를 전달할 경우 카테고리 미지정으로 카테고리별 조회에서 조회할 수 없다.)
            - 직접 판매
                - 상품 등록 시 상품 상태는 판매중(SELLING) 상태로 등록된다.
                - 추가 필요 입력 정보: 품질
                - 거래 방식 등록
                    - 거래 가능 정보들을 배열로 입력한다. 거래 방식의 유형이 동일한 정보는 여러개 등록하더라도 하나만 등록된다(직거래 제외).
                      - 객체별 필요 입력 정보: 거래 방식(택배(PARCEL), 반값 택배(CONVENIENCE), 직거래(MEETING)) 중 선택, 추가 금액(없을 시 0 혹은 미입력)
                          - 거래방식이 직거래인 경우 추가 필요 정보: 직거래 위치, 시간(등록 시간으로부터 1시간 이전 시간이면 등록되지 않는다. 위치, 시간의 쌍은 고유해야 한다.)
                          - 직거래의 경우 추가 필요 정보가 다르다면 다른 내용으로 취급, 등록된다.
                      - 추가 금액은 배송비 등 거래 방식 별 발생하는 추가 금액이다(배송비, 포장비 등)
            - 위탁 판매
                - 상품 등록 시 상품 상태는 위탁 신청(CONSIGNMENT_APPLY)로 등록된다. 이 상태로는 판매 상품 조회 시 조회되지 않는다.
                - 추가 필요 입력 정보: 판매자 주소 식별번호(미입력시 상품 등록자의 대표 주소 식별번호 자동입력), 판매자 계좌 식별번호(미입력시 상품 등록자의 대표 계좌 식별번호 자동 입력)
    - 관리자의 위탁 상품 관리
        - 신청된 위탁 정보 조회
    ...
    - 판매자의 위탁 상품 전체 조회(상품 진행 상태로 필터 가능)
        - 판매자가 자신이 위탁한 물건들의 목록을 조회할 수 있다.
    ...
    - 판매중인 상품 목록 조회(카테고리별, 품질, 가격 필터 가능)
        - 상태가 판매중(SELLING)이거나 거래진행중(PROCESSING)이고, 거래 방식이 등록되어있는 모든 상품 목록을 불러온다.
        - 조회 정보: 판매자(위탁 판매일 경우 담당자) 식별번호, 이미지(가장 먼저 등록된 이미지 한 장), 상품명, 품질, 가격, 상품 설명, 해당 상품의 거래 가능 방식 종류, 등록 시간, 수정 시간
    - 상품 정보 조회
        - 필요 정보: 상품 식별번호
        - 상품의 상세 정보를 불러온다.
        - 조회 정보: 판매자(위탁 판매일 경우 담당자) 식별번호, 이미지 리스트, 상품명, 품질, 가격, 상품 설명, 상태, 후기, 거래방식(상태가 판매중인 경우 구체적인 거래 방식 정보, 아닌 경우 빈 리스트), 등록 시간, 수정 시간
    - 상품 검색(카테고리, 품질, 가격 필터 가능)
        - 필요 정보: 검색 단어
        - 입력한 검색 단어가 상품명 또는 상품 설명에 포함되는 판매중이거나 거래진행중인 상품 목록 조회
- ### 거래 관리
    - 구매자의 거래 신청
        - 구매를 원하는 사용자는 거래 방식을 확인하고, 가능한 거래 방식 식별번호로 거래 신청을 보낼 수 있다.
        - 필요 입력 정보: 거래 방식 식별 번호
        - 거래 신청 시 거래의 상태는 신청(APPLIED) 상태가 된다.
        - 상세정보 식별번호와 구매자 식별번호의 쌍이 고유해야 한다.
        - 신청하려는 거래 방식이 택배일 경우
            - 필요 입력 정보: 환불 계좌 식별 번호(미입력 시 대표 계좌), 수신자명, 수신자 연락처, 수신자 주소 식별번호(미입력시 대표 주소)
        - 신청하려는 거래 방식이 반값 택배일 경우
            - 필요 입력 정보: 환불 계좌 식별 번호(미입력시 대표 계좌), 편의점 지점, 수신자명, 수신자 연락처
    - 담당자의 상품별 거래 신청 확인
    ...
    - 담당자의 거래 승인
        - 필요 입력 정보: 거래 식별 번호
        - 상품의 상태가 SELLING이고, 거래가 신청 상태인 거래 신청을 승인(APPROVED)할 수 있다.
    ...
    - 개별 거래 내역 조회
        ...
        - 필요 입력 정보: 거래 식별번호
        - 조회 정보: 상품 정보, 거래 방식 정보, 거래 진행 단계, 입금 정보(택배 혹은 반값 택배일 경우), 환불 정보(택배 혹은 반값 택배일 경우), 구매자 주소 정보(택배일 경우), 편의점 지점(반값 택배일 경우), 발송 정보(택배, 반값 택배일 경우)
- ### 거래 방식 등록
  - 거래 방식 등록
    - 필요 입력 정보: 상품 식별 번호, 거래 가능 정보(배열로 입력)
      - 객체별 필요 입력 정보: 거래 방식(택배(PARCEL), 반값 택배(CONVENIENCE), 직거래(MEETING)) 중 선택, 추가 금액(없을 시 0 혹은 미입력)
      - 거래방식이 직거래인 경우 추가 필요 정보: 직거래 위치, 시간(등록 시간으로부터 1시간 이후 시간이어야 등록 가능하다. 위치, 시간의 쌍은 고유해야 한다.)
      - 상품 식별 번호가 동일하고 거래 방식의 유형이 동일한 정보는 여러개 등록하더라도 하나만 등록된다.
      - 직거래의 경우 추가 필요 정보가 다르다면 다른 내용으로 취급, 등록된다.
    - 추가 금액은 배송비 등 거래 방식 별 발생하는 추가 금액이다(배송비, 포장비 등)
...
- ### 신고/건의 관리
    - 신고/건의사항 등록
        ...
        - 필요 입력 정보: (신고하는 경우) 신고할 대상의 식별번호, 분류(ACCUSE, SUGGEST), 구체적인 내용
    ...
    - 신고/건의사항 전체 조회(상태별 필터 가능)
        - 일반 사용자는 자신이 등록한 신고/건의 전체 목록을 불러올 수 있다.
        - 관리자는 등록된 신고/건의 전체 목록을 조회할 수 있다.
    - 신고/건의사항 개별 조회
        - 필요 입력 정보: 신고/건의 식별번호
        - 일반 사용자는 자신이 등록한 신고/건의사항만 조회 가능하다.
        - 조회 정보: 신고자 식별번호, 피고인 식별번호, 이미지 리스트, 분류, 내용, 답변, 상태, 등록 시간
- ### 이미지 관리
    - 이미지 등록
        - Amazon S3을 통해 이미지를 등록한다.
        - 필요 입력정보: 사용처 식별번호, 사용처(상품(PRODUCT), 신고/건의(REPORT), 후기(REVIEW))
        - 이 때 이미지명은 "등록시간-장 수"로 저장된다.
    - 이미지 삭제
        - 필요 입력정보: 이미지 식별번호
